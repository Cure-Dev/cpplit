...
public:
	...
	_builtin_object_tuple* eval(environment env) {
		std::vector<semantic_object*> result;
		for (auto item : this->ITEMS) {
			result.push_back(dynamic_cast<semantic_object*> ((item)->eval(env)));
		}
...
public:
	...
	semantic_node* eval(environment env) {
		...
		if (dynamic_cast<semantic_function*> (callee) != NULL) { //
			return (dynamic_cast<semantic_function*> (callee)) -> call(this->CALLER->eval(env));
		}
		else {
			throw "cannot call";
		}
